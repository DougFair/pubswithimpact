...
import PapersDisplay2 from "./PapersDisplay2";
...
class App extends Component {
    ...
    this.setState(
      ...
      () => {
        if (!this.state.inputedDate1) {
          dateParams = `${this.state.dateMinus1}"[Date - Publication] : "3000"[Date - Publication])`;
          dateParams2 = `${this.state.dateMinus7}"[Date - Publication] : "${this.state.dateMinus2}"[Date - Publication])`;
        } else if (this.state.inputedDate1 && !this.state.inputedDate2) {
          dateParams = `${this.state.inputedDate1}"[Date - Publication] : "3000"[Date - Publication])`;
        } else if (this.state.inputedDate1 && this.state.inputedDate2) {
          dateParams = `${this.state.inputedDate1}"[Date - Publication] : "${this.state.inputedDate2}"[Date - Publication])`;
        }
  ...
  noJournalSearch = () => {
    ...
    this.setState({ loading: true });
    const dateParams = `${this.state.dateMinus7}"[Date - Publication] : "3000"[Date - Publication])`;
    const urlEncoded = encodeURIComponent(urlunencoded);
    const dateParamsEncoded = encodeURIComponent(dateParams);
    const url = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmode=json&retmax=10000&term=${urlEncoded}${dateParamsEncoded}&api_key=${process.env.REACT_APP_PUBMED_API_KEY}`;
    axios
